...
class Solution:
    ...
    def __init__(self, plan: List, n_courses: int, table_sizes: List[int], fixed: pd.DataFrame):
        self.plan = plan
    ...
    def improve_solution(self, greedy: bool, verbose=False):
        ...
        conflicts = np.argwhere(
            [[all([x in self.plan[course][table] for x in issue]) for table in range(len(self.table_sizes))] for course
             in
        ...
        for conflict in np.random.permutation(conflicts):
            ...
            for person in np.random.permutation(issue):
                if person not in self.fixed[self.fixed['course']==course]['id'].values:
                    ...
                    for other_table in np.random.permutation(list(set((range(len(self.table_sizes)))) - set([table]))):
                        for other_person in np.random.permutation(self.plan[course][other_table]):
                            if other_person not in self.fixed[self.fixed['course'] == course]['id'].values:
                                ...
                                new_solution = Solution(new_plan, self.n_courses, self.table_sizes, self.fixed)
                                new_score = new_solution.get_score()
                                if verbose:
                                    print('conflict: ' + str(conflict) + '. Switching person ' + str(
                                        person) + ' in course ' + str(course) +
                                          ' from table ' + str(table) + ' to ' + str(other_table) + ' with person ' + str(
                                        other_person) +
